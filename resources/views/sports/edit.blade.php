...
--}}
    ...
    <form action="{{route('taches.update',$tache->id)}}" method="POST">
        ...
        <div>
            ...
            <input type="date" name="expiration" id="expiration" value="{{ $tache->expiration }}" placeholder="aaaa-mm-jj">
        </div>
        <div>
            {{-- la catégorie --}}
            <label for="categorie"><strong>Catégorie</strong></label>
            <input type="text" class="form-control" id="categorie" name="categorie" value="{{ $tache->categorie}}">
        </div>
        <div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="6" class="form-control" placeholder="Description..">{{ $tache->description }}</textarea>
        </div>
        <div>
